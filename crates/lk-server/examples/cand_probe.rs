...
async fn main() {
    ...
    let room = format!("diag-stun-{}", uuid::Uuid::new_v4().simple().to_string().chars().take(6).collect::<String>());
    ...
    .unwrap();
    turn_creds();
    let url = format!("ws://216.48.182.132:7880/rtc?access_token={token}&reconnect=false&auto_subscribe=true");
    ...
    for _ in 0..30 {
        let msg = match tokio::time::timeout(std::time::Duration::from_secs(5), ws.next()).await {
            Ok(Some(Ok(m))) => m,
            _ => { el += 5; if el > 60 { break; } continue; }
        };
...
fn turn_creds() {
    ...
    println!("TURN_USERNAME={}", lk_server::turn::turn_username(key, sid, expiry));
    println!("TURN_PASSWORD={}", lk_server::turn::turn_password(secret, sid, expiry));
    println!("TURN_EXPIRY={}", expiry);
}

fn crate_turn_expiry() -> i64 {
    std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs() as i64 + 300
}
